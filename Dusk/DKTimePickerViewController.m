...
#import "DKTimePickerViewController.h"
#import "EFCircularSlider.h"

@interface DKTimePickerViewController ()

@property (weak, nonatomic) IBOutlet UILabel *labelTime;

@end

@implementation DKTimePickerViewController {
...
- (void)viewDidLoad
{
    ...
    minuteSlider.currentValue = [components minute];
    ...
    hourSlider.currentValue = [components hour];
    ...
    [self.labelTime setText:[NSString stringWithFormat:@"%d:%02d", [components hour], [components minute]]];
}

-(void)hourDidChange:(EFCircularSlider*)slider {
    int newVal = (int)slider.currentValue ? (int)slider.currentValue : 12;
    NSString* oldTime = self.labelTime.text;
    NSRange colonRange = [oldTime rangeOfString:@":"];
    self.labelTime.text = [NSString stringWithFormat:@"%d:%@", newVal, [oldTime substringFromIndex:colonRange.location + 1]];
}
...
-(void)minuteDidChange:(EFCircularSlider*)slider {
    ...
    NSString* oldTime = self.labelTime.text;
    NSRange colonRange = [oldTime rangeOfString:@":"];
    self.labelTime.text = [NSString stringWithFormat:@"%@:%02d", [oldTime substringToIndex:colonRange.location], newVal];
}
